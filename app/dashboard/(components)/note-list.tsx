      ...
      if (target && target.files?.[0]) {
        ...
        try {
          ...
          if (result.success) {
            ...
            setNotes((prevState: INote[]): INote[] => [
              ...prevState.map((i: INote) =>
                i._id === noteId ? (result.data as INote) : i
              ),
            ]);
          } else {
  ...
  useEffect(() => {
    if (noteItems && noteItems?.[0]?.note) {
      if (!noteItems?.[0]?.isPinned) {
        ...
        setNotes((prev) => [
          ...prev.filter((i) => i._id !== noteItems?.[0]._id),
        ]);
        emptyNotes();
      }
    }
  }, [noteItems, isUpdate, emptyNotes, mutateNotes]); // Depend on 'noteItems' to add new notes
